-- Migration SQL for NotFat - Prisma Schema to Supabase (FIXED ORDER)
...
-- 1. Subscriptions table (no dependencies)
...
-- 2. Nutrition Goals table (no dependencies)
...
-- 3. Hydration Goals table (no dependencies)
...
-- 4. Health Settings table (no dependencies)
...
-- 5. User Activity Profile table (no dependencies)
...
-- 6. Profiles table (references other tables, but we'll add FKs later)
CREATE TABLE IF NOT EXISTS profiles (
  ...
  nutrition_goals_id UUID,
  hydration_goals_id UUID,
  ...
  health_settings_id UUID,
  user_activity_profile_id UUID
);

-- 7. Meals table (references profiles)
...
-- 8. Food Items table (references meals)
...
-- 9. Water Logs table (references profiles)
...
-- 10. Scan Events table (references profiles)
...
-- 11. Task Queue table (references profiles)
...
-- 12. Coach Insights table (references profiles)
...
-- 13. Contribution Queue table (references profiles)
...
ALTER TABLE profiles 
  ADD CONSTRAINT fk_profiles_nutrition_goals 
    FOREIGN KEY (nutrition_goals_id) REFERENCES nutrition_goals(id) ON DELETE SET NULL,
  ADD CONSTRAINT fk_profiles_hydration_goals 
    FOREIGN KEY (hydration_goals_id) REFERENCES hydration_goals(id) ON DELETE SET NULL,
  ADD CONSTRAINT fk_profiles_subscription 
    FOREIGN KEY (subscription_id) REFERENCES subscriptions(id) ON DELETE SET NULL,
  ADD CONSTRAINT fk_profiles_health_settings 
    FOREIGN KEY (health_settings_id) REFERENCES health_settings(id) ON DELETE SET NULL,
  ADD CONSTRAINT fk_profiles_user_activity_profile 
    FOREIGN KEY (user_activity_profile_id) REFERENCES user_activity_profile(id) ON DELETE SET NULL;
...
ALTER TABLE water_logs 
  ADD CONSTRAINT fk_water_logs_user_id 
    FOREIGN KEY (user_id) REFERENCES profiles(id) ON DELETE CASCADE;

ALTER TABLE scan_events 
  ADD CONSTRAINT fk_scan_events_user_id 
    FOREIGN KEY (user_id) REFERENCES profiles(id) ON DELETE CASCADE;

ALTER TABLE task_queue 
  ADD CONSTRAINT fk_task_queue_user_id 
    FOREIGN KEY (user_id) REFERENCES profiles(id) ON DELETE CASCADE;

ALTER TABLE coach_insights 
  ADD CONSTRAINT fk_coach_insights_user_id 
    FOREIGN KEY (user_id) REFERENCES profiles(id) ON DELETE CASCADE;

ALTER TABLE contribution_queue 
  ADD CONSTRAINT fk_contribution_queue_user_id 
    FOREIGN KEY (user_id) REFERENCES profiles(id) ON DELETE CASCADE,
  ADD CONSTRAINT fk_contribution_queue_approved_by 
